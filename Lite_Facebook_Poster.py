...
def connect(headless=0) :
    ...
    print("التوجه إلى موقع الفيسبوك لتسجيل الدخول")
...
def writePost(group) :
    ...
    try :
        ...
        for i in range(len(images)) :
            ...
            # Click add image
            imgLoc = "C:\\Users\\Nova\\OneDrive\\Desktop\\py\\facebook-group-post\\"+imgFolder+"\\"+images[i]
        ...
        # Add text
        driver.find_element_by_tag_name('textarea').send_keys(msg)
        sleep(1)

        # Submit
        buttons = driver.find_elements_by_name('view_post')
        buttons[0].click()
        #buttons = driver.find_elements_by_xpath('//*[@id="root"]/table/tbody/tr/td/div[2]/div[1]/a')
        #buttons[0].click()
        print("✅ posted", end=' ')
    except :
        print("⛔️ error", end=' ')
...
def collectGroupURLS(filename='liteURL'):
    ...
    g_urls = driver.find_elements_by_xpath('//li[@class="bn"]//a')
